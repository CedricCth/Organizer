...
| `./adr/` | Why we chose what we chose (one file per ADR) | Every time a non-trivial technical decision is made. |
...
3. **If it implies new tech**, write a new ADR under `./adr/` *first*. Don't bolt on a new library silently.
...
1. **Open [`./adr/README.md`](./adr/README.md)** and copy the ADR template. Create a new file `./adr/NNNN-<slug>.md` for your decision (next sequential number).
...
1. Reading `CLAUDE.md`.
2. Reading `FEATURES.md`, `TECH_STACK.md`, `ARCHITECTURE.md`, and the ADRs in `./adr/` (or skimming for what's relevant to the request).
...
- **Planning sessions** (Cowork) — slow, doc-heavy. Output: changes to `FEATURES.md` / new ADRs in `./adr/`.
